...
export function CronAdminClient({
  ...
  return (
    <div>
      ...
      {error ? (
        <p
          className={`${embedded ? "" : "mt-6"} border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800`}
        >
          {error}
        </p>
      ) : null}
        ...
        <>
            ...
            <div className="mt-6 border border-amber-300 bg-amber-50 px-4 py-4">
              ...
              <p className="mt-1 text-sm text-amber-900/90">
                ...
                {status.cronControl.pausedUntil
                  ? new Date(status.cronControl.pausedUntil).toLocaleString(
                      "es-ES",
                    )
                  : "—"}
          ...
          <section className="mt-8 admin-card p-6">
            ...
            <p className="mt-1 max-w-xl text-sm text-stone-600">
              ...
              aquí puedes forzar un lote.
            </p>
            {status?.settings ? (
              <p className="mt-3 text-sm text-stone-600">
                Umbral: dto. ≥ {status.settings.telegramMinDiscountPercent}%
                {" · "}lote cada {status.settings.telegramBatchHours} h
                {status.settings.telegramFlushLimit != null
                  ? ` · máx. ${status.settings.telegramFlushLimit}/envío`
                  : ""}
                {status?.telegramBatchDue ? " · lote listo" : ""}
                {!loadingStatus &&
                status?.telegramNextFlushAt &&
                !status?.telegramBatchDue
                  ? ` · próximo ${new Date(status.telegramNextFlushAt).toLocaleString("es-ES")}`
                  : ""}
                {status.settings.source === "env"
                  ? " (valores de entorno)"
                  : ""}
              </p>
            ) : null}
        ...
        <>
          <div className={showMonitor ? "mt-10" : ""}>
            ...
            <p className="mt-1 max-w-2xl text-sm text-stone-600">
              Precios vigila el catálogo ya indexado. Flash descubre ASINs
              nuevos. Telegram publica la cola pendiente.
            </p>
          </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            <section className="admin-card flex flex-col p-5">
              ...
              <p className="mt-2 flex-1 text-sm text-stone-600">
                Lote de productos empezando por los más antiguos. Detecta bajadas
                y encola al canal si cumplen el % mínimo.
              </p>
              <div className="mt-5 flex flex-wrap items-end gap-3">
                <AdminField label="Límite" className="!w-24 shrink-0">
                  <input
                    type="number"
                    min="1"
                    max="50"
            ...
            <section className="admin-card flex flex-col border-amber-200 bg-amber-50/30 p-5">
              ...
              <p className="mt-2 flex-1 text-sm text-stone-600">
                Busca ASINs nuevos en Gold Box / Deals. Lo ya indexado lo cubre
                «Revisar precios».
              </p>
              <div className="mt-5 flex flex-wrap items-end gap-3">
                <AdminField label="Límite" className="!w-24 shrink-0">
            ...
            <section className="admin-card flex flex-col p-5">
              ...
              <div className="mt-5">
                ...
                <button
                  type="button"
                  disabled={flushingTelegram || loadingStatus || running}
                  onClick={() => void flushTelegramNow()}
                  className="admin-btn admin-btn-primary"
                >
                  {flushingTelegram ? "Enviando…" : "Enviar lote"}
                </button>
              </div>
            </section>
          </div>
          ...
          {flashResult?.ok ? (
            <section className="mt-8 space-y-6">
              <h3 className="font-display text-lg text-ink">Resultado Flash</h3>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <ResultStat
                  label="Actualizados"
                  value={flashResult.updated ?? 0}
                />
                <ResultStat
                  label="Nuevos mínimos"
                  value={flashResult.newLows ?? 0}
                />
              </div>
              ...
              <p className="text-sm text-stone-500">
                ...
                {flashResult.discovery?.usedSimulation
                  ...
                  : ""}
                {" · "}Telegram cola{" "}
                {flashResult.channelNotificationsQueued ?? 0} / enviadas{" "}
                {flashResult.channelNotificationsSent ?? 0} / omitidas{" "}
